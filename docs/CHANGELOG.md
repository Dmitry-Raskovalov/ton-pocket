...
### Исправлено
- `src/services/validation/check-new-recipient.ts`:
  - Предупреждение о новом получателе теперь имеет `blocking: true`, что обязывает пользователя подтвердить риск через чекбокс перед отправкой.
- `src/components/WarningList.tsx`:
  - Исправлена логика `onAllBlockingConfirmed`: при отсутствии blocking-предупреждений автоматически вызывается `true`, разблокируя кнопку отправки (ранее вызывался `false`, что навсегда блокировало кнопку).

---

## [2026-04-02] - Актуализация плана проекта

### Изменено
- `docs/PLAN.md`:
  - Все выполненные задачи в разделах 1-10 (пункты 1.1, 2.1, 2.2, 3.1, 3.2, 3.3, 4.4, 9.2) отмечены как выполненные (`[x]`).
  - Статус разделов 2.1 «Формирование общего ТЗ на дизайн» и 2.2 «Поэкранное описание для дизайна» обновлен с `TODO` на `DONE`.
  - План приведен в полное соответствие с текущим состоянием кодовой базы и реализованным функционалом.

## [2026-04-02] - Реализация удаления кошелька

### Добавлено
- `src/screens/Settings/SettingsScreen.tsx`:
  - Реализован функционал «Delete Wallet» в секции Danger Zone.
  - Добавлено диалоговое окно подтверждения удаления с предупреждением о необходимости сохранения мнемонической фразы.
  - При удалении вызываются `clearVault()` (очистка localStorage) и `clearWallet()` (сброс состояния store), после чего пользователь перенаправляется на экран Welcome.

---

## [2026-04-02] - Улучшение производительности и верстки MainScreen

### Изменено
- `src/screens/MainScreen.tsx`:
  - Верстка переведена на `h-dvh` (динамическая высота вьюпорта) для предотвращения внешнего скролла страницы.
  - Список транзакций теперь скроллится независимо внутри выделенной области (`overflow-y-auto`).
  - Кнопка "Load more" заменена на **Infinite Scroll** через `IntersectionObserver`: подгрузка происходит автоматически при достижении конца списка.
  - Хедер и блок баланса зафиксированы в верхней части экрана.
...
### Изменено
- `src/screens/SendScreen.tsx` — визуальные улучшения:
  - Единый индикатор прогресса (3 сегмента) теперь отображается на всех этапах отправки (Input, Confirm, Result).
  - На каждом шаге подсвечивается прогресс: Шаг 1 (1/3), Шаг 2 (2/3), Шаг 3 (3/3).
  - Удалён дублирующий заголовок на экране подтверждения; теперь используется единый sticky header.
...
### Исправлено
- `src/crypto/kdf.ts` — улучшена проверка доступности Argon2 (теперь реально запускается тестовое хеширование для проверки загрузки WASM).
- `src/crypto/kdf.ts` — исправлен `distPath` на `'/'` для деривации Argon2, что необходимо для корректной работы в браузере.
- `.env.example` — обновлён `VITE_TON_ENDPOINT` (добавлен `/jsonRPC`), что исправляет 405 Method Not Allowed при запросах к некоторым Testnet нодам.
- `src/screens/SendScreen.tsx` — исправлено формирование `secretKey` (удалена ошибочная конкатенация с `publicKey`), что приводило к ошибкам подписи при отправке средств.
