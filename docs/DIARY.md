...
### Наблюдения
- Существующие тесты (425 в 36 файлах) покрывали все основные модули. Аудит выявил 3 модуля без тестов: check-new-recipient, ValidationPipeline, useWallet.
- TonService.ts — заглушка (все методы кидают Not implemented), тестировать нечего.
- check-new-recipient косвенно тестировался через validate-send.test.ts, но не было прямых тестов с моками addressBook.

### Решения
- Создан новый каталог `src/integration/` для round-trip тестов (create→export, import→export, changePassword→export).
- Добавлены edge case тесты для vault (пустая строка, 10KB, перезапись, отсутствующие поля), kdf (короткий хеш), password-strength (пробелы, unicode, длинный пароль), balance (10B TON).
- Увеличен timeout для changePassword тестов — PBKDF2 с 600k итерациями требует >5s в test env.

### Проблемы
- PBKDF2 с 600000 итерациями делает тесты changePassword медленными. Увеличение timeout до 30s решает проблему, но общий прогон тестов занимает ~56s.

---

## [2026-04-15] - Исправление блокировки кнопки отправки при предупреждении о новом адресе

### Проблемы
- При отправке TON на новый адрес появлялось предупреждение «Вы впервые отправляете TON на этот адрес», но кнопка «Confirm & Send TON» оставалась навсегда заблокированной — невозможно было подтвердить и отправить.
- Корневая причина состояла из двух частей:
  1. Предупреждение `new_recipient` имело `blocking: false` — чекбокс подтверждения не отображался.
  2. `WarningList` при отсутствии blocking-предупреждений вызывал `onAllBlockingConfirmed(false)`, что навсегда блокировало кнопку.

### Решения
- **`check-new-recipient.ts`**: изменён `blocking: false` → `blocking: true`. Теперь предупреждение о новом адресе требует обязательного подтверждения через чекбокс.
- **`WarningList.tsx`**: добавлен ранний return — если среди предупреждений нет ни одного blocking, вызывается `onAllBlockingConfirmed(true)`, автоматически разблокируя кнопку отправки.
- Обновлены тесты в `WarningList.test.tsx` и `validate-send.test.ts`.

---

## [2026-04-02] - Синхронизация плана разработки

### Наблюдения
- В ходе разработки многие задачи были реализованы, но не отмечены в `docs/PLAN.md`. Это затрудняло оценку реального прогресса и планирование следующих этапов (раздела 11 «Тестирование» и далее).

### Решения
- **PLAN.md Sync**: Проведен аудит всех разделов до пункта 11. Все фактически реализованные фичи (крипто-слой, UI экраны, сервисы TON) теперь официально закрыты в плане.
- **Статусы**: Разделы дизайна (2.1, 2.2), которые были завершены еще в начале проекта, теперь имеют корректный статус `DONE`.
- **Точность**: Исправлены мелкие пропуски в отметках юнит-тестов (9.2) и сервиса транзакций (4.4).
