...
describe('MainScreen — data fetching', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(useTransactionStore).mockReturnValue(makeStoreState());
    vi.mocked(getFilteredTransactions).mockReturnValue([]);
  ...
  it('calls getTransactions on mount', async () => {
    mockGetTransactions.mockResolvedValue([]);
    renderScreen();
    await waitFor(() => expect(mockGetTransactions).toHaveBeenCalledOnce());
  });

  it('calls getBalance on mount', async () => {
    mockGetTransactions.mockResolvedValue([]);
    renderScreen();
    await waitFor(() => expect(mockGetBalance).toHaveBeenCalledOnce());
  });

  it('calls setTransactions with fetched data and hasMore=false for ≤20 items', async () => {
    const tx = makeTx({ hash: 'fetched-tx' });
    mockGetTransactions.mockResolvedValue([tx]);
    renderScreen();
    await waitFor(() =>
      expect(mockSetTransactions).toHaveBeenCalledWith([tx], false)
    );
  });

  it('sets hasMore=true when API returns 21 items (PAGE_SIZE+1)', async () => {
    const txs = Array.from({ length: 21 }, (_, i) =>
      makeTx({ hash: `tx-${i}`, lt: String(i) })
    );
    mockGetTransactions.mockResolvedValue(txs);
    renderScreen();
    await waitFor(() =>
      expect(mockSetTransactions).toHaveBeenCalledWith(txs.slice(0, 20), true)
    );
  });
});
